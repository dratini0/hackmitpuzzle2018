...
class Block:
    ...
    def hash_block(self):
        if not hasattr(self, "partialHash") or self.partialHash is None:
            self.partialHash = hashlib.sha256()
            self.partialHash.update(str(self.timestamp) + 
                                    str(self.transactions) + 
                                    str(self.previous_hash))

        sha = self.partialHash.copy()
